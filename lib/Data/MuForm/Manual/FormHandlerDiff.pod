...
doing something similar in the renderer, but it's not yet been developed.
...
Removed 'defaults' on 'process'. Use init_object instead or implement via user code.
...
DBIC Model: the 'item' has been renamed to 'model' and is now cleared on process like the other attributes.
