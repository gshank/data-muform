...
takes about the same amount of time (most of extra rendering related time in FH is
...
Renamed 'init_contains' to 'init_instance'
...
You no longer need to use C<$field->_set_value($value)>. Just use C<$field->value($value)>.
...
Password, PasswordConf, IntRange. The DateTime field has been renamed to 'CompoundDateTime'.
...
Added 'build_field_id' as a possible form method to build fields ids.
