...
code and functionality has stood the test of time and use very well, and has been moved
to MuForm with only minor changes.
...
The 'item' has been renamed to 'model' and is now cleared on process like the other attributes.
...
See the rendering documentation. There will be 'layouts'.

No 'has_block', since that's purely a rendering thing. There will some way of
doing something similar in the renderer, but it's not been developed yet.
...
If you want a different filter, create your own renderer.
...
Removed flag 'use_init_obj_over_item'. Just use init_values in your process call instead.
...
Password, PasswordConf, IntRange.  Could possibly be supplied in a compatibility library. Let me
