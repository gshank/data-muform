...
$form = Repeatable::Form::User->new( model => $user );
ok( $form->field('addresses')->field('0')->field('country')->has_options,
    'addresses has options from new' );

$form->process( model => $user, params => {} );
ok( $form->field('addresses')->field('0')->field('country')->has_options,
...
$form2->process( model => $schema->resultset('User')->new_result( {} ),
    params => {} );
...
$user = $form->model;
is( $user->user_name, 'Joe Smith', 'created model');
...
$form->process($fif);
is( $form->model->search_related( 'addresses', {city => 'Middle City'} )->first->country->printable_name, 'Graustark', 'updated addresses');
...
$form->process($params);
my $new_address = $form->model->search_related('addresses', { address_id => {'>', 3} })->single;
